...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let bobsGenerationInterval = null; // Para controlar a geração passiva de Bobs
...
function calculateOfflineBobs() {
    ...
    const bobsPerHour = {
        1: 100,  // Nível 1: 50 Bobs/30min = 100 Bobs/hora
        2: 200,  // Nível 2: 100 Bobs/30min = 200 Bobs/hora
        3: 120,
        4: 300
    };
...
function getRandomPhrase(phraseType) {
    ...
    // <-- NOVO: Função para gerar Bobs passivamente -->
    function startPassiveBobsGeneration() {
        if (bobsGenerationInterval) clearInterval(bobsGenerationInterval);
        // Gera 1 Bob a cada 5 segundos
        bobsGenerationInterval = setInterval(() => {
    ...
    function initializeGame(mode = 'single', options = {}) {
        ...
        startGameLoop();
        startPassiveBobsGeneration(); // <-- NOVO: Inicia a geração de Bobs
    ...
    function showStartScreen() { 
        ...
        bobsMineScreen.style.display = 'none'; // CORREÇÃO: Esconde a tela da mina
        bobsShopScreen.style.display = 'none'; // CORREÇÃO: Esconde a tela da loja
        renderSaveSlots(); 
    }
...
async function handleMineUpgrade() {
    ...
    // CORREÇÃO: Garante que mineData exista antes de ser usado
    if (!currentUser.mineData) {
        currentUser.mineData = { level: 1, lastCollectedTimestamp: Date.now() };
    }

    // Define os custos para cada nível de upgrade.
    const upgradeCosts = {
        1: 100, 
        2: 500,  
        3: 2000 
...
async function executeTargetedSpell(spellId, event) {
    ...
    if (effect.type === 'area_stun') {
        ...
        // CORREÇÃO: Lógica para chuva de flechas
        // Efeito visual
        for (let i = 0; i < effect.arrowCount; i++) {
            setTimeout(() => {
                const arrow = document.createElement('div');
                arrow.className = 'arrow-projectile';
                const angle = Math.random() * Math.PI * 2;
                const distance = Math.random() * effect.radius;
                arrow.style.left = `${x + Math.cos(angle) * distance}px`;
                arrow.style.top = `${y + Math.sin(angle) * distance - 100}px`; // Começa de cima
                gameArea.appendChild(arrow);
                setTimeout(() => arrow.remove(), 400); // Remove após a animação
            }, Math.random() * 500); // Espalha a "chuva" ao longo do tempo
        }
        
        // Aplica dano aos monstros na área
        monsters.forEach(m => {
            const distSq = Math.pow((m.x + m.element.offsetWidth / 2) - x, 2) + Math.pow((m.y + m.element.offsetHeight / 2) - y, 2);
            if (distSq <= effect.radius * effect.radius) {
                // O dano total é o número de flechas vezes o dano por flecha
                const totalDamage = effect.arrowCount * effect.damagePerArrow;
                damageMonster(m, totalDamage);
            }
        });
    }
